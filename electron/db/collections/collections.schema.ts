...
import { media } from '../utils/utils.schema'
...
/**
 * Sidebar collection order (`Global.COLLECTION_INDICES` / `COLLECTIONS_ORDER_KEY`).
 * One row per `(bucket_key, collection_id)`; `sort_index` is the drag order.
 */
export const collectionOrder = sqliteTable(
	'collection_order',
	{
		bucketKey: text('bucket_key').notNull().default('sidebar'),
	...
	(t) => [
		primaryKey({ columns: [t.bucketKey, t.collectionId] }),
		index('collection_order_bucket_sort_idx').on(t.bucketKey, t.sortIndex)
	]
)
